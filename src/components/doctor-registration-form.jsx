import HospitalNameFormContainer from "../components/hospital-name-form-container";
import IconFontAwesomeFreeSolidH from "../components/icon-font-awesome-free-solid-h";
import { handle_doctor_registration } from "../utils/Contract";
...
const DoctorRegistration = () => {
  ...
  const signIN =async  () => {
    setUsername(doctor_name + randomNumberGenerator());
    const procedd = await handle_doctor_registration(username, doctor_name, licence_ID, hospital_address, department, password);
    if(procedd){
  ...
  const onButtonmuicontainedTextClick = useCallback(() => {
    if(logged_in)   navigate("/hospital-list");
  }, [navigate]);
    ...
    <div className="w-full relative [background:linear-gradient(180deg,_#5811b0,_#000_36.4%)] h-[67.5rem] overflow-hidden text-left text-[1.75rem] text-white font-electrolize">
      ...
      <form>
        <div className="w-[40rem] flex flex-col items-start justify-start gap-[1rem_0rem] text-left text-[1.75rem] text-white font-electrolize">
          ...
          <input onChange={(e) => setDoctor_name(e.target.value)} className="self-stretch rounded-61xl bg-white flex flex-row items-start justify-start py-[1.5rem] px-[2rem] text-[1.5rem] text-blue-1-9 border-[2px] border-solid border-dark-blue-1-8" id="hospital_form_input" placeholder="Enter Doctor Name"></input>
        ...
        <div className="w-[40rem] flex flex-col items-start justify-start gap-[1rem_0rem] text-left text-[1.75rem] text-white font-electrolize">
          ...
          {/* <label htmlFor="hospital_form_input">{hospitalInputLabels}</label> */}
          <input onChange={(e) => setHOspital_address(e.target.value)} className="self-stretch rounded-61xl bg-white flex flex-row items-start justify-start py-[1.5rem] px-[2rem] text-[1.5rem] text-blue-1-9 border-[2px] border-solid border-dark-blue-1-8" id="hospital_form_input" placeholder="Enter Hospital Address"></input>
        ...
        <div className="w-[40rem] flex flex-col items-start justify-start gap-[1rem_0rem] text-left text-[1.75rem] text-white font-electrolize">
          ...
          {/* <label htmlFor="hospital_form_input">{hospitalInputLabels}</label> */}
          <input onChange={(e) => setDepartment(e.target.value)} className="self-stretch rounded-61xl bg-white flex flex-row items-start justify-start py-[1.5rem] px-[2rem] text-[1.5rem] text-blue-1-9 border-[2px] border-solid border-dark-blue-1-8" id="hospital_form_input" placeholder="Enter Department"></input>
